...
# Optional LangSmith tracing (requires langchain callbacks package)
try:
...
# Create a LangSmith tracer if the LANGSMITH_API_KEY env var is set.
# Put your LangSmith API key in a .env file as:
# LANGSMITH_API_KEY=your_key_here
# or export it in the shell before running: export LANGSMITH_API_KEY=your_key_here
langsmith_tracer = None
if LangSmithTracer is not None and os.getenv("LANGSMITH_API_KEY"):
    try:
        langsmith_tracer = LangSmithTracer(project_name="Researcher-Agent")
        print("LangSmith tracer enabled (traces will appear in your LangSmith project).")
    except Exception as e:
        print("Failed to create LangSmith tracer:", e)
        langsmith_tracer = None
elif LangSmithTracer is not None:
    print("LangSmith tracer available but LANGSMITH_API_KEY not set. Add it to your .env to enable tracing.")
        ...
        if isinstance(message_chunk, AIMessageChunk):
            ...
            if message_chunk.tool_call_chunks:
                ...
                if message_chunk.content:
                    ...
                    # Normalize chunk content to a string (handle str, list, dict, None)
                    if isinstance(raw, list):
                        try:
                            normalized = "".join(
                                x if isinstance(x, str) else (x.get("content", "") if isinstance(x, dict) else str(x))
                                for x in raw
                            )
                        except Exception:
                            normalized = "".join(map(str, raw))
                    else:
                        normalized = "" if raw is None else (raw if isinstance(raw, str) else str(raw))
...
async def main():
    ...
    try:
        # Include LangSmith tracer in the RunnableConfig callbacks if available
        if langsmith_tracer:
